...
public class ConstraintsIDEPlugin extends AbstractUIPlugin {
	...
	private ConstraintLanguagesSpec specification;
	...
	throws Exception {
		...
		specification = (ConstraintLanguagesSpec) specificationResource.getContents().get( 0 );
	...
	throws ResourceAccessException, IOException {
		...
		if ( specUri.isPlatformPlugin() ) {
			...
			specification = (ConstraintLanguagesSpec) resSet.getResource( specUri, true ).getContents().get( 0 );
		}
		else {
			final Resource resource = specification.eResource();
			resource.save( null );
	...
	private ConstraintLanguagesSpec initDefaultSpecification( final URI p_destUri ) 
	throws ResourceAccessException, IOException {
		...
		if ( bundleIdDefaultModel != null ) {
			...
			if ( resSet.getURIConverter().exists( uri, null ) ) {
				...
				return (ConstraintLanguagesSpec) modelRes.getContents().get( 0 );
			}
		}
	...
	private ConstraintLanguagesSpec createDefaultConstraintLanguageSpecification() {
		final ConstraintLanguagesSpec spec = ConstraintsFactory.eINSTANCE.createConstraintLanguagesSpec();
		...
		spec.getOwnedLanguages().add( natLanguage );
	...
	throws Exception {
		for ( final ConstraintLanguage language : specification.getOwnedLanguages() ) {
	...
	throws Exception {
		if ( p_location != null && !"".equals( p_location ) ) {
			p_language.getOwnedLibraries().clear();
			final String languageDir = FILE_SEPARATOR_CHAR + ( p_language.getLibraryLocation() == null ? p_language.getName().toLowerCase() : p_language.getLibraryLocation() );
			...
			if ( directoryPath.exists() ) {
				...
				for ( final File libraryFile : libFiles ) {
					...
					try {
						...
						library.setOwnedExpression( libExpression );
	
						if ( libraryFile.getParent().endsWith( "default" ) ) {
							library.setDefault( true );
						}
	
						p_language.getOwnedLibraries().add( library );
					}
					catch ( final IOException p_ex ) {
						logger.error( toString(), p_ex );
					}
	...
	public final Collection<ConstraintLanguage> getDefinedConstraintLanguages() {
		return specification.getOwnedLanguages();
	}
	...
	throws CoreException {
		...
		for ( final ConstraintLanguage language : getSpecification().getOwnedLanguages() ) {
			if ( language instanceof FormalConstraintLanguage ) {
				final FormalConstraintLanguage formalLanguage = (FormalConstraintLanguage) language;
				final ServiceDescriptor interpDescr = formalLanguage.getOwnedInterpreter();
				...
				try {
					...
					final IConstraintInterpretationAS service = (IConstraintInterpretationAS) interpDescr.getServiceClass().newInstance();
	...
	throws CoreException {
		...
		for ( final ConstraintLanguage language : getSpecification().getOwnedLanguages() ) {
			if ( language instanceof FormalConstraintLanguage ) {
				final FormalConstraintLanguage formalLanguage = (FormalConstraintLanguage) language;
				final ServiceDescriptor persistDescr = formalLanguage.getOwnedPersister();
				...
				if ( persistDescr != null ) {
					...
					try {
						...
						services.put( 	formalLanguage, 
										(IConstraintPersistenceAS) persistDescr.getServiceClass().newInstance() );
					}
					catch ( final Throwable p_th ) {
						throw new CoreException( new Status( 	IStatus.ERROR, 
	...
	public ConstraintLanguagesSpec getSpecification() {
